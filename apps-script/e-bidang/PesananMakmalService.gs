 ...
 * makmal). Guru/Ketua Panitia panitia berkenaan membuat pesanan, memilih
 * SATU Makmal fizikal (SENARAI_MAKMAL) serta masa mula/tamat penggunaan.
 * Setiap pesanan disalurkan HANYA kepada Pembantu Makmal yang dijaga kepada
 * Makmal berkenaan (USERS.MakmalDijaga) — Pembantu Makmal lain tidak nampak
 * pesanan itu. KETUA_PEMBANTU_MAKMAL (+ Admin/Ketua Bidang) melihat & boleh
 * memproses SEMUA pesanan merentasi semua Makmal. Borang formal (dengan
 * logo sekolah) dijana & dicetak di klien (Index.html) daripada data
 * apiButiranPesanan().
 * ========================================================================= */
...
const HEADER_PESANAN_MAKMAL = ['IDPesanan', 'Panitia', 'NamaGuru', 'NoKPGuru', 'Kelas',
  ...
  'DiprosesOleh', 'TarikhDiproses', 'CatatanPembantu', 'Makmal', 'MasaMula', 'MasaTamat'];
...
/* PEMBANTU_MAKMAL biasa hanya boleh lihat pesanan bagi Makmal yang dijaganya sendiri
   (sesi.makmal — SENARAI, seorang Pembantu Makmal boleh dijaga > 1 makmal). */
function bolehLihatPesanan(sesi, pesanan) {
  if (PERANAN_LIHAT_SEMUA_PESANAN.indexOf(sesi.peranan) !== -1) return true;
  if (sesi.peranan === ROLE_PEMBANTU_MAKMAL) return (sesi.makmal || []).indexOf(pesanan.Makmal) !== -1;
...
function apiSenaraiPesanan(p) {
  ...
  if (PERANAN_LIHAT_SEMUA_PESANAN.indexOf(sesi.peranan) !== -1) {
    ...
    if (p.makmal) senarai = senarai.filter(ps => ps.Makmal === p.makmal);
  } else if (sesi.peranan === ROLE_PEMBANTU_MAKMAL) {
    senarai = bacaSheetSebagaiObjek(SHEET_PESANAN_MAKMAL).filter(ps => (sesi.makmal || []).indexOf(ps.Makmal) !== -1);
...
function apiSimpanPesanan(p) {
  ...
  const makmal = String(p.makmal || '').trim();
  if (SENARAI_MAKMAL.indexOf(makmal) === -1) return ralat('Sila pilih Makmal yang sah.');
  ...
  const masaMula = String(p.masaMula || '').trim();
  const masaTamat = String(p.masaTamat || '').trim();
  ...
  if (!kelas || !tajukEksperimen || !tarikhDiperlukan || !masaMula || !masaTamat) {
    return ralat('Sila lengkapkan kelas, tajuk eksperimen, tarikh diperlukan, masa mula dan masa tamat.');
  }
  if (masaTamat <= masaMula) return ralat('Masa tamat mesti selepas masa mula.');
  ...
  catatAudit(sesi, 'TAMBAH', 'PESANAN_MAKMAL', idPesanan, 'Pesanan makmal baharu: ' + tajukEksperimen + ' (' + panitia + ', ' + makmal + ', ' + kelas + ')');
...
/* Pembantu Makmal (Makmal sendiri sahaja) / Ketua Pembantu Makmal / Admin / Ketua Bidang
   menukar status & catatan pemprosesan. */
function apiKemaskiniStatusPesanan(p) {
  const sesi = wajibPeranan(p.token, PERANAN_PROSES_PESANAN);
  if (sesi.success === false) return sesi;

  const pesanan = cariBarisMengikutId(SHEET_PESANAN_MAKMAL, 'IDPesanan', p.idPesanan);
  if (!pesanan) return ralat('Pesanan tidak dijumpai.');
  if (!bolehLihatPesanan(sesi, pesanan)) return ralat('Anda tidak mempunyai kebenaran untuk memproses pesanan ini.');
